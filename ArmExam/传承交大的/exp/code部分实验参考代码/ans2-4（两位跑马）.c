...
#define GPIO_FLASHTIME						30				//30mS，GPIO定时时长
...
volatile uint16_t systick_10ms_couter=SYSTICK_FREQUENCY/100;	//10ms和100ms计时器
volatile uint16_t systick_100ms_couter=SYSTICK_FREQUENCY/10; 	//10ms和100ms计时器
  ...
  IntMasterEnable();	//处理器中断使能	
	...
	while (1)
	{
		if (systick_10ms_status) //10ms定时到
		{
			...
			if (++gpio_flash_cnt >= 10)              //define GPIO_FLASHTIME			30	
			{
				gpio_flash_cnt			= 0;                             //每100ms闪一次
				
				//PF0灯反转
				GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_0, ~GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_0 )); 
			}

			//数码管跑马灯（利用10ms定时刷新数码管，可以让视觉同时看到两个码管值）
		...
		if (systick_100ms_status) //100ms定时到
		{
			...
			{
				i2c_flash_cnt				= 0;           //define	I2C_FLASHTIME							500		

				if (++cnt >= 0x8)                    //0.5s cnt++ 移位
					cnt = 0;
