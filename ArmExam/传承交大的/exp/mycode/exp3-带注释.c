  ...
  SysTickPeriodSet(ui32SysClock/SYSTICK_FREQUENCY);
   ...
   IntMasterEnable();		
	...
	while (1)
	{
		...
		{
			uart_receive_char = (uint8_t)UARTCharGet(UART0_BASE);         //UARTCharGet函数  得到字符
...
void UARTStringPut(const char *cMessage)  //字符串传递函数 送出
{
	while(*cMessage!='\0')
		UARTCharPut(UART0_BASE,*(cMessage++)); //传递字符 送出
}
...
void S800_UART_Init(void)           //UART初始化
{
  ...
  GPIOPinTypeUART(GPIO_PORTA_BASE, GPIO_PIN_0 | GPIO_PIN_1);
	...
	// Configure the UART for 115,200, 8-N-1 operation. //配置波特率  8个数据位  1个停止位 0个奇偶校验位
