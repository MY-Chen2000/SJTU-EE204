...
int main(void)
{
  ...
  SysTickPeriodSet(ui32SysClock/SYSTICK_FREQUENCY);
	...
	IntEnable(INT_UART0);                     //使能中断
  UARTIntEnable(UART0_BASE, UART_INT_RX | UART_INT_RT);	//Enable UART0 RX,TX interrupt  接受中断与超时中断
  IntMasterEnable();		
...
*/
void UART0_Handler(void)   //中断程序
{
	int32_t uart0_int_status;
  uart0_int_status 		= UARTIntStatus(UART0_BASE, true);		// Get the interrrupt status. 获取端口当前中断状态 true表示需要获取屏蔽的中断状态
	                                                                                                   // false表示需要获取原始的中断状态

  UARTIntClear(UART0_BASE, uart0_int_status);								//Clear the asserted interrupts 清除端口的中断

  while(UARTCharsAvail(UART0_BASE))    											// Loop while there are characters in the receive FIFO.
  {                                                                       //UARTCharsAvail(UART0_BASE) 确认端口接受的FIFO中是否有字符
		///Read the next character from the UART and write it back to the UART.
    UARTCharPutNonBlocking(UART0_BASE,UARTCharGetNonBlocking(UART0_BASE));//把接受的字符发送出去
		GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1,GPIO_PIN_1 );		 //点亮PN1
		
//		Delay(1000); //过了1000烧不进去?
	}
	GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1,0 );	          //关闭PN1
}
