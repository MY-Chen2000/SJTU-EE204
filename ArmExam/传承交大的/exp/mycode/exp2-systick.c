...
int main(void)
{
	volatile uint16_t	i2c_flash_cnt,gpio_flash_cnt;   //定义后默认为0
   ...
   //ui32SysClock = SysCtlClockFreqSet((SYSCTL_XTAL_25MHZ |SYSCTL_OSC_MAIN | SYSCTL_USE_PLL |SYSCTL_CFG_VCO_480), 120000000);;		
	...
	SysTickPeriodSet(ui32SysClock/SYSTICK_FREQUENCY);  //??SysTick ??????    SYSTICK_FREQUENCY 
	SysTickEnable();//?????
	//SysTickDisable();//?????
	//uint32_t SysTickValueGet(void) ????????
	SysTickIntEnable();//??systick??
  IntMasterEnable();//???????
	//void SysTickIntDisable(void) //??systick??	
...
*/
void SysTick_Handler(void)               //   100ms  
{
	if (systick_100ms_couter	!= 0)
		systick_100ms_couter--;
	else
	{
		systick_100ms_couter	= SYSTICK_FREQUENCY/10; //到0后重新开始 SYSTICK_FREQUENCY=1000
		systick_100ms_status 	= 1;                  //计数完status变为1
	}
	
	if (systick_10ms_couter	!= 0)             //   10ms  
		systick_10ms_couter--;
	else
	{
