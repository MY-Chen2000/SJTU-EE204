...
int main(void)
{
  ...
  IntMasterEnable();		
	...
	IntPriorityGroupingSet(3);														//Set all priority to pre-emtption priority  总共三位优先级分配三位给抢占
	                                                                                                 //分配0位给子优先级
	IntPrioritySet(INT_UART0,3);													//Set INT_UART0 to highest priority
	
	IntPrioritySet(FAULT_SYSTICK,0xe);									//Set INT_SYSTICK to lowest priority 优先级只看最高三位
...
void SysTick_Handler(void)
{
	...
	while (GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_0) == 0)   //sw1  //优先级低
	{
...
//		Delay(1000);
	...
	while (GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_1) == 0)    //sw2  //优先级高    //用这个把前面那个while屏蔽
		GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1,GPIO_PIN_1);	
