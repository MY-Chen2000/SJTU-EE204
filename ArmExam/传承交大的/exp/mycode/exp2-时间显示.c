  ...
  SysTickPeriodSet(ui32SysClock/SYSTICK_FREQUENCY);
   ...
   IntMasterEnable();		
	...
	while (1)
	{
		...
		if (systick_100ms_status)  //改为1s1跳
		{  
			systick_100ms_status	= 0;
							
				for (i=0;i<50;i++) {                                        //循环50次(经验数值,根据实际情况改))
				result 							= I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[num1]);	//write port 1 				
				result 							= I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,0x1);	//write port 2
				Delay(20000);
		    result 							= I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[num2]);	//write port 1 				
				result 							= I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,0x2);	//write port 2
				Delay(20000);
				result 							= I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,0x40);	//write port 1 				
				result 							= I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,0x4);	//write port 2
				Delay(20000);
				result 							= I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[num3]);	//write port 1 				
				result 							= I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,0x8);	//write port 2
				Delay(20000);
        result 							= I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[num4]);	//write port 1 				
				...
				if(num3>=6){num2++;num3=0;}
				if(num2>=10){num1++;num2=0;}
				if(num1>=6){num1=0;}
...
void SysTick_Handler(void)
{
	...
	{
		systick_100ms_couter	= 1000/1;    //改为1s
