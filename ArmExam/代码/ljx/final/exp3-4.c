...
int i=0;
int32_t UARTget[8]={0};
int32_t CLASS[8]={'A','T','+','C','L','A','S','S'};
int32_t Class[8]={'a','t','+','c','l','a','s','s'};
int32_t STUDENTCODE[14]={'A','T','+','S','T','U','D','E','N','T','C','O','D','E'};
int32_t Studentcode[14]={'a','t','+','s','t','u','d','e','n','t','c','o','d','e'};
bool claflag=1, stuflag=1;	
int main(void)
{
...
*/
  ...
  {
		///Read the next character from the UART and write it back to the UART.
		UARTget[i]=UARTCharGetNonBlocking(UART0_BASE);
		if(UARTget[i]!=CLASS[i]&&UARTget[i]!=Class[i]){
			claflag=0;
		}
		if(UARTget[i]!=STUDENTCODE[i]&&UARTget[i]!=Studentcode[i]){
			stuflag=0;
		}
		i++;
		if(i==14) break;
	...
	if(claflag){
		UARTStringPut((uint8_t *)"CLASS F1703304\n");
		i=0;
	}else	if(stuflag){
		if(i==14){
		UARTStringPut((uint8_t *)"CODE 517021910521\n");
		i=0;}
	}else{
		i=0;
	}
